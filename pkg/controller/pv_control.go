...
// PVControlInterface manages PVs used in CellCluster
type PVControlInterface interface {
	PatchPVReclaimPolicy(*v1alpha1.CellCluster, *corev1.PersistentVolume, corev1.PersistentVolumeReclaimPolicy) error
	UpdateMetaInfo(*v1alpha1.CellCluster, *corev1.PersistentVolume) (*corev1.PersistentVolume, error)
}
...
func (rpc *realPVControl) PatchPVReclaimPolicy(cc *v1alpha1.CellCluster, pv *corev1.PersistentVolume, reclaimPolicy corev1.PersistentVolumeReclaimPolicy) error {
	pvName := pv.GetName()
	...
	rpc.recordPVEvent("patch", cc, pvName, err)
	return err
}

func (rpc *realPVControl) UpdateMetaInfo(cc *v1alpha1.CellCluster, pv *corev1.PersistentVolume) (*corev1.PersistentVolume, error) {
	ns := cc.GetNamespace()
	ccName := cc.GetName()
	...
	if pvcRef == nil {
		glog.Warningf("PV: [%s] doesn't have a ClaimRef, skipping, CellCluster: %s/%s", pvName, ns, ccName)
		return pv, nil
	}
	...
	if err != nil {
		...
		glog.Warningf("PV: [%s]'s PVC: [%s/%s] doesn't exist, skipping. CellCluster: %s", pvName, ns, pvcName, ccName)
		return pv, nil
	}
	...
	if pv.Labels[label.NamespaceLabelKey] == ns &&
		...
		glog.V(4).Infof("pv %s already has labels and annotations synced, skipping. CellCluster: %s/%s", pvName, ns, ccName)
		return pv, nil
	}
	...
	err = retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		if updateErr == nil {
			glog.Infof("PV: [%s] updated successfully, CellCluster: %s/%s", pvName, ns, ccName)
			return nil
		}
		glog.Errorf("failed to update PV: [%s], CellCluster %s/%s, error: %v", pvName, ns, ccName, err)
	...
	rpc.recordPVEvent("update", cc, pvName, err)
	return updatePV, err
}

func (rpc *realPVControl) recordPVEvent(verb string, cc *v1alpha1.CellCluster, pvName string, err error) {
	ccName := cc.GetName()
	if err == nil {
		reason := fmt.Sprintf("Successful%s", strings.Title(verb))
		msg := fmt.Sprintf("%s PV %s in CellCluster %s successful",
			strings.ToLower(verb), pvName, ccName)
		rpc.recorder.Event(cc, corev1.EventTypeNormal, reason, msg)
	} else {
		reason := fmt.Sprintf("Failed%s", strings.Title(verb))
		msg := fmt.Sprintf("%s PV %s in CellCluster %s failed error: %s",
			strings.ToLower(verb), pvName, ccName, err)
...
func (fpc *FakePVControl) PatchPVReclaimPolicy(_ *v1alpha1.CellCluster, pv *corev1.PersistentVolume, reclaimPolicy corev1.PersistentVolumeReclaimPolicy) error {
...
func (fpc *FakePVControl) UpdateMetaInfo(cc *v1alpha1.CellCluster, pv *corev1.PersistentVolume) (*corev1.PersistentVolume, error) {
	defer fpc.updatePVTracker.inc()
	if fpc.updatePVTracker.errorReady() {
		defer fpc.updatePVTracker.reset()
