...
// PodControlInterface manages Pods used in CellCluster
type PodControlInterface interface {
	// TODO change this to UpdatePod
	UpdateMetaInfo(*v1alpha1.CellCluster, *corev1.Pod) (*corev1.Pod, error)
	DeletePod(*v1alpha1.CellCluster, *corev1.Pod) error
	UpdatePod(*v1alpha1.CellCluster, *corev1.Pod) (*corev1.Pod, error)
}
...
func (rpc *realPodControl) UpdatePod(cc *v1alpha1.CellCluster, pod *corev1.Pod) (*corev1.Pod, error) {
	ns := cc.GetNamespace()
	ccName := cc.GetName()
	podName := pod.GetName()
	...
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if updateErr == nil {
			glog.Infof("Pod: [%s/%s] updated successfully, CellCluster: [%s/%s]", ns, podName, ns, ccName)
			return nil
		}
	...
	rpc.recordPodEvent("update", cc, podName, err)
	return updatePod, err
}

func (rpc *realPodControl) UpdateMetaInfo(cc *v1alpha1.CellCluster, pod *corev1.Pod) (*corev1.Pod, error) {
	ns := pod.GetNamespace()
	podName := pod.GetName()
	labels := pod.GetLabels()
	ccName := cc.GetName()
	if labels == nil {
		return pod, fmt.Errorf("pod %s/%s has empty labels, CellCluster: %s", ns, podName, ccName)
	}
	_, ok := labels[label.InstanceLabelKey]
	if !ok {
		return pod, fmt.Errorf("pod %s/%s doesn't have %s label, CellCluster: %s", ns, podName, label.InstanceLabelKey, ccName)
	}
	clusterID := labels[label.ClusterIDLabelKey]
	memberID := labels[label.MemberIDLabelKey]
	storeID := labels[label.StoreIDLabelKey]
	pdClient := rpc.pdControl.GetPDClient(cc)
	if labels[label.ClusterIDLabelKey] == "" {
		cluster, err := pdClient.GetCluster()
		if err != nil {
			return pod, fmt.Errorf("failed to get cell cluster info from pd, CellCluster: %s/%s, err: %v", ns, ccName, err)
		}
		clusterID = cluster
	}
	...
	case label.PDLabelVal:
		if labels[label.MemberIDLabelKey] == "" {
			...
			if err != nil {
				return pod, fmt.Errorf("failed to get pd members info from pd, CellCluster: %s/%s, err: %v", ns, ccName, err)
			}
	...
	case label.StoreLabelVal:
		if labels[label.StoreIDLabelKey] == "" {
			...
			if err != nil {
				return pod, fmt.Errorf("failed to get stores info from pd, CellCluster: %s/%s, err: %v", ns, ccName, err)
			}
	...
	if labels[label.ClusterIDLabelKey] == clusterID &&
		...
		glog.V(4).Infof("pod %s/%s already has cluster labels set, skipping. CellCluster: %s", ns, podName, ccName)
		return pod, nil
	}
	...
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		updatePod, updateErr = rpc.kubeCli.CoreV1().Pods(ns).Update(pod)
		if updateErr == nil {
			glog.V(4).Infof("update pod %s/%s with cluster labels %v successfully, CellCluster: %s", ns, podName, labels, ccName)
			return nil
		}
		glog.Errorf("failed to update pod %s/%s with cluster labels %v, CellCluster: %s, err: %v", ns, podName, labels, ccName, updateErr)
...
func (rpc *realPodControl) DeletePod(cc *v1alpha1.CellCluster, pod *corev1.Pod) error {
	ns := cc.GetNamespace()
	ccName := cc.GetName()
	podName := pod.GetName()
	err := rpc.kubeCli.CoreV1().Pods(ns).Delete(podName, nil)
	if err != nil {
		glog.Errorf("failed to delete Pod: [%s/%s], CellCluster: %s, %v", ns, podName, ccName, err)
	} else {
		glog.V(4).Infof("delete Pod: [%s/%s] successfully, CellCluster: %s", ns, podName, ccName)
	}
	rpc.recordPodEvent("delete", cc, podName, err)
	return err
}

func (rpc *realPodControl) recordPodEvent(verb string, cc *v1alpha1.CellCluster, podName string, err error) {
	ccName := cc.GetName()
	if err == nil {
		reason := fmt.Sprintf("Successful%s", strings.Title(verb))
		msg := fmt.Sprintf("%s Pod %s in CellCluster %s successful",
			strings.ToLower(verb), podName, ccName)
		rpc.recorder.Event(cc, corev1.EventTypeNormal, reason, msg)
	} else {
		reason := fmt.Sprintf("Failed%s", strings.Title(verb))
		msg := fmt.Sprintf("%s Pod %s in CellCluster %s failed error: %s",
			strings.ToLower(verb), podName, ccName, err)
		rpc.recorder.Event(cc, corev1.EventTypeWarning, reason, msg)
	}
}
...
var (
	...
	TestName          string = "cell-cluster"
	TestComponentName string = "store"
	TestPodName       string = "pod-1"
	TestManagedByName string = "cell-operator"
...
func (fpc *FakePodControl) UpdateMetaInfo(_ *v1alpha1.CellCluster, pod *corev1.Pod) (*corev1.Pod, error) {
...
func (fpc *FakePodControl) DeletePod(_ *v1alpha1.CellCluster, pod *corev1.Pod) error {
...
func (fpc *FakePodControl) UpdatePod(_ *v1alpha1.CellCluster, pod *corev1.Pod) (*corev1.Pod, error) {
	defer fpc.updatePodTracker.inc()
	if fpc.updatePodTracker.errorReady() {
		defer fpc.updatePodTracker.reset()
		return nil, fpc.updatePodTracker.err
