...
// PVCControlInterface manages PVCs used in CellCluster
type PVCControlInterface interface {
	UpdateMetaInfo(*v1alpha1.CellCluster, *corev1.PersistentVolumeClaim, *corev1.Pod) (*corev1.PersistentVolumeClaim, error)
	UpdatePVC(*v1alpha1.CellCluster, *corev1.PersistentVolumeClaim) (*corev1.PersistentVolumeClaim, error)
	DeletePVC(*v1alpha1.CellCluster, *corev1.PersistentVolumeClaim) error
}
...
func (rpc *realPVCControl) DeletePVC(cc *v1alpha1.CellCluster, pvc *corev1.PersistentVolumeClaim) error {
	ns := cc.GetNamespace()
	ccName := cc.GetName()
	pvcName := pvc.GetName()
	err := rpc.kubeCli.CoreV1().PersistentVolumeClaims(cc.GetNamespace()).Delete(pvcName, nil)
	if err != nil {
		glog.Errorf("failed to delete PVC: [%s/%s], CellCluster: %s, %v", ns, pvcName, ccName, err)
	}
	glog.V(4).Infof("delete PVC: [%s/%s] successfully, CellCluster: %s", ns, pvcName, ccName)
	rpc.recordPVCEvent("delete", cc, pvcName, err)
	return err
}

func (rpc *realPVCControl) UpdatePVC(cc *v1alpha1.CellCluster, pvc *corev1.PersistentVolumeClaim) (*corev1.PersistentVolumeClaim, error) {
	ns := cc.GetNamespace()
	ccName := cc.GetName()
	pvcName := pvc.GetName()
	...
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		if updateErr == nil {
			glog.Infof("update PVC: [%s/%s] successfully, CellCluster: %s", ns, pvcName, ccName)
			return nil
		}
		glog.Errorf("failed to update PVC: [%s/%s], CellCluster: %s, error: %v", ns, pvcName, ccName, updateErr)
	...
	rpc.recordPVCEvent("update", cc, pvcName, err)
	return updatePVC, err
}

func (rpc *realPVCControl) UpdateMetaInfo(cc *v1alpha1.CellCluster, pvc *corev1.PersistentVolumeClaim, pod *corev1.Pod) (*corev1.PersistentVolumeClaim, error) {
	ns := cc.GetNamespace()
	ccName := cc.GetName()
	pvcName := pvc.GetName()
	podName := pod.GetName()
	...
	if pvc.Labels[label.ClusterIDLabelKey] == clusterID &&
		...
		glog.V(4).Infof("pvc %s/%s already has labels and annotations synced, skipping, CellCluster: %s", ns, pvcName, ccName)
		return pvc, nil
	}
	...
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		updatePVC, updateErr = rpc.kubeCli.CoreV1().PersistentVolumeClaims(ns).Update(pvc)
		if updateErr == nil {
			glog.V(4).Infof("update PVC: [%s/%s] successfully, CellCluster: %s", ns, pvcName, ccName)
			return nil
		}
		glog.Errorf("failed to update PVC: [%s/%s], CellCluster: %s, error: %v", ns, pvcName, ccName, updateErr)
...
func (rpc *realPVCControl) recordPVCEvent(verb string, cc *v1alpha1.CellCluster, pvcName string, err error) {
	ccName := cc.GetName()
	if err == nil {
		reason := fmt.Sprintf("Successful%s", strings.Title(verb))
		msg := fmt.Sprintf("%s PVC %s in CellCluster %s successful",
			strings.ToLower(verb), pvcName, ccName)
		rpc.recorder.Event(cc, corev1.EventTypeNormal, reason, msg)
	} else {
		reason := fmt.Sprintf("Failed%s", strings.Title(verb))
		msg := fmt.Sprintf("%s PVC %s in CellCluster %s failed error: %s",
			strings.ToLower(verb), pvcName, ccName, err)
...
func (fpc *FakePVCControl) DeletePVC(_ *v1alpha1.CellCluster, pvc *corev1.PersistentVolumeClaim) error {
...
func (fpc *FakePVCControl) UpdatePVC(_ *v1alpha1.CellCluster, pvc *corev1.PersistentVolumeClaim) (*corev1.PersistentVolumeClaim, error) {
...
func (fpc *FakePVCControl) UpdateMetaInfo(_ *v1alpha1.CellCluster, pvc *corev1.PersistentVolumeClaim, pod *corev1.Pod) (*corev1.PersistentVolumeClaim, error) {
	defer fpc.updatePVCTracker.inc()
	if fpc.updatePVCTracker.errorReady() {
		defer fpc.updatePVCTracker.reset()
		return nil, fpc.updatePVCTracker.err
