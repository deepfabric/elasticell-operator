...
func TestLabelNew(t *testing.T) {
	...
	l := New()
	g.Expect(l[NameLabelKey]).To(Equal("cell-cluster"))
	g.Expect(l[ManagedByLabelKey]).To(Equal("cell-operator"))
}
...
func TestLabelProxy(t *testing.T) {
	g := NewGomegaWithT(t)

	l := New()
	l.Proxy()
	g.Expect(l.IsProxy()).To(BeTrue())
}
