...
func (pdmm *metaManager) Sync(cc *v1alpha1.CellCluster) error {
	ns := cc.GetNamespace()
	...
	for _, pod := range pods {
		...
		_, err := pdmm.podControl.UpdateMetaInfo(cc, pod)
		if err != nil {
			return err
		}
		if component := pod.Labels[label.ComponentLabelKey]; component != label.PDLabelVal && component != label.StoreLabelVal {
		...
		_, err = pdmm.pvcControl.UpdateMetaInfo(cc, pvc, pod)
		if err != nil {
			return err
		}
		...
		_, err = pdmm.pvControl.UpdateMetaInfo(cc, pv)
		if err != nil {
			return err
		}
	}
...
func (pdmm *metaManager) resolvePVCFromPod(pod *corev1.Pod) (*corev1.PersistentVolumeClaim, error) {
	...
	for _, vol := range pod.Spec.Volumes {
		switch vol.Name {
		case v1alpha1.PDMemberType.String(), v1alpha1.StoreMemberType.String():
...
func (fmm *FakeMetaManager) Sync(_ *v1alpha1.CellCluster) error {
	if fmm.err != nil {
		return fmm.err
	}
	return nil
