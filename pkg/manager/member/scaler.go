...
type Scaler interface {
	...
	ScaleOut(*v1alpha1.CellCluster, *apps.StatefulSet, *apps.StatefulSet) error
	// ScaleIn scales in the cluster
	ScaleIn(*v1alpha1.CellCluster, *apps.StatefulSet, *apps.StatefulSet) error
}
...
func (gs *generalScaler) deleteDeferDeletingPVC(cc *v1alpha1.CellCluster,
	...
	return skipReason, gs.pvcControl.DeletePVC(cc, pvc)
}
...
func ordinalPodName(memberType v1alpha1.MemberType, ccName string, ordinal int32) string {
	return fmt.Sprintf("%s-%s-%d", ccName, memberType, ordinal)
}
