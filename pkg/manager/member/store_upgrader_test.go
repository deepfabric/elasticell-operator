...
func TestStoreUpgraderUpgrade(t *testing.T) {
	...
	type testcase struct {
		...
		expectFn            func(*GomegaWithT, *v1alpha1.CellCluster, *apps.StatefulSet, map[string]*corev1.Pod)
	}
	...
	testFn := func(test *testcase, t *testing.T) {
		...
		cc := newCellClusterForStoreUpgrader()
	...
	tests := []*testcase{
		{
			...
			expectFn: func(g *GomegaWithT, cc *v1alpha1.CellCluster, newSet *apps.StatefulSet, pods map[string]*corev1.Pod) {
		...
		},
		{
			...
			errExpectFn: func(g *GomegaWithT, err error) {
				g.Expect(err).To(HaveOccurred())
				g.Expect(err.Error()).To(Equal("cellcluster: [default/upgrader]'s store pod: [upgrader-store-1] is evicting leader"))
			},
			expectFn: func(g *GomegaWithT, cc *v1alpha1.CellCluster, newSet *apps.StatefulSet, pods map[string]*corev1.Pod) {
				g.Expect(*newSet.Spec.UpdateStrategy.RollingUpdate.Partition).To(Equal(int32(2)))
			},
...
func newStoreUpgrader() (Upgrader, *controller.FakePDControl, *controller.FakePodControl, podinformers.PodInformer) {
...
func newCellClusterForStoreUpgrader() *v1alpha1.CellCluster {
