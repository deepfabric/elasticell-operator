...
func TestPDUpgraderUpgrade(t *testing.T) {
	...
	testFn := func(test *testcase) {
		...
		cc := newCellClusterForPDUpgrader()
	...
	tests := []testcase{
		...
		},
		{
			...
			changeFn: func(cc *v1alpha1.CellCluster) {
				cc.Status.PD.Synced = true
				cc.Status.PD.Members[pdPodName(upgradeCcName, 2)] = v1alpha1.PDMember{Name: pdPodName(upgradeCcName, 2), Health: false}
			},
			changePods:        nil,
			transferLeaderErr: false,
			errExpectFn: func(g *GomegaWithT, err error) {
				g.Expect(err.Error()).To(Equal(fmt.Sprintf("cellcluster: [default/upgrader]'s pd upgraded pod: [%s] is not ready", pdPodName(upgradeCcName, 2))))
			},
			expectFn: func(g *GomegaWithT, cc *v1alpha1.CellCluster, newSet *apps.StatefulSet) {
				g.Expect(cc.Status.PD.Phase).To(Equal(v1alpha1.UpgradePhase))
...
func newCellClusterForPDUpgrader() *v1alpha1.CellCluster {
